...
if [ -z "$container_list" ]; then # コンテナ起動してなければ実行その１
    xhost + # ディスプレイ表示用、これをexport DISPLAYの前に実行する必要がある。
fi
...
if [ -z "$container_list" ]; then # コンテナ起動してなければ起動その２
...
./docker/internal/docker_exec.sh
