...
if [ "$UPDATE_VERSION" = true ]; then
    ...
    # Update BASE_IMAGE_VERSION for the current target config
    update_base_image_version "docker/environment/${CUB_TARGET}.conf" "$CONFIG_BASE_IMAGE_VERSION"
    
    echo "Base build successful! Updated BASE_IMAGE_VERSION to ${CONFIG_BASE_IMAGE_VERSION} for ${CUB_TARGET}.conf"
fi
