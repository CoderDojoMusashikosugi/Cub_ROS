...
・USBデバイス名の固定化を行なっていること  
下記のスクリプトを実行し、/dev/ttyATOM、/dev/ttySLC1が存在すればよし。  
[USBデバイスの固定化](/scripts/mcub_host_settings.sh)

・シリアルポートを有効化していること  (m5ATOMを使用する場合は不要)  
 有効化手順  
