...
1. **6 軸 IMU バイナリストリーミング (240 Hz)**
   - 加速度・角速度・温度データをハードウェア FIFO（閾値 1: 1サンプル即時）経由で低遅延読み出し
...
3. **IMU DRDY 割り込みエッジキャプチャ & 遅延補正 (D27 ピン)**
   - IMU の DRDY（データ準備完了）信号を **D27 ピン** に入力し、立ち上がりエッジのマイクロ秒時刻を割り込みキャプチャ
   - キャプチャしたエッジ時刻に内部フィルタ遅延補正用オフセット（`DRDY_DELAY_OFFSET_US`）を加算して UTC タイムスタンプを算出
   - **未接続時の安全設計**: D27 ピンは内部プルダウン（`INPUT_PULLDOWN`）に設定。D27 が未接続（オープン）の場合はノイズによる誤割り込みを防ぎ、従来通り SPI 読み出し完了時の時刻へ自動フォールバック

4. **時刻同期ステータスフラグ (`status`)**
   - パケット内の `status` バイト（ビットフラグ）で同期状況を通知：
     ...
     - `0x20` (Bit 5): DRDY 割り込みエッジ（D27）によるタイムスタンプ取得が有効

---

## ハードウェア配線 (UM982, IMU DRDY と Spresense 拡張ボード)

Spresense 拡張ボードの I/O 電圧ジャンパ **JP1 を必ず 3.3V 側に設定** してください。

| Spresense 拡張ボード | 接続先デバイス / ピン | 用途 |
|---|---|---|
| **GND** | UM982 GND / 共通 GND | 共通 GND |
| **D0 (Serial2 RX)** | UM982 TXD (COM1/2/3) | NMEA / `#EVENTFLAGA` メッセージ受信 |
| **D1 (Serial2 TX)** | UM982 RXD (同じ COM) | UM982 初期化設定コマンド送信 |
| **D3** | UM982 1PPS | 1秒周期のパルス信号（時刻同期） |
| **D4** | UM982 EVENT / EVE | （任意）イベント同時入力ピン（モーメンタリボタン接続） |
| **D27** | IMU DRDY 信号ピン | IMU データ準備完了割り込み入力（未接続時は自動フォールバック） |

### DRDY 遅延補正定数 (`DRDY_DELAY_OFFSET_US`)
スケッチ上部（`spresense_imu_ext_timesync.ino`）で定義されています。
```cpp
constexpr int32_t DRDY_DELAY_OFFSET_US = 0; // マイクロ秒単位（正負どちらも可）
```
IMU 内部のデジタルフィルタ群遅延（group delay）や信号線遅延をオフセット補正したい場合に調整してください。
